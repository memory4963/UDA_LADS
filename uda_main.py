...
if args.DATA.LOAD_CACHED:
    ...
    if args.METHOD.NORMALIZE:
        ...
        if args.DATA.UNLABELED_DATA != 0: # TODO: LUO: will this norm change the distribution of test set?
            unlabeled_features /= np.linalg.norm(unlabeled_features, axis=-1, keepdims=True)
# Load the model
...
# TODO: LUO: temporarily not used, remember to adapt this to UDA
...
if args.EXP.AUGMENTATION != None and args.EXP.AUGMENTATION != 'None':
    ...
    if "LADS" in args.EXP.AUGMENTATION or 'Directional' in args.EXP.AUGMENTATION:
        if 'UDA' in args.EXP.AUGMENTATION:
